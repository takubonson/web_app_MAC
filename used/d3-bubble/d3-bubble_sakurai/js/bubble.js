...
const generateChart = data => {
    ...
    const detailPage = d3.select(".detail");
    ...
    const circle = node.append('circle')
        ...
        .on("focus", function(event, d){
            ...
            detailPage.select("img").attr("src", d.data.img);
            detailPage.select("a").attr("href", d.data.link).text(d.data.name);
            detailPage.select("span").attr("class", d.data.category).text(d.data.category);
        })
