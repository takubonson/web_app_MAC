...
const file = 'mini_market_cap.json';
const offset = 60;
const blank_size = 30;
const min_year = 2001;
const max_year = 2003;
...
const generateChart = data => {
    ...
    var bubble = data => d3.pack()
        ...
        .padding(2)(d3.hierarchy({ children: selected_data }).sum(function(d){
            if (!d.children){
                // console.log(d.value[0][1])
                return d.value[0][1]
                // ここを調節する
            }else{
                // console.log(d.value)
...
/*         .on('mousemove', e => tooltip.style('top', `${e.pageY}px`)
    ...
    function dragged(event) {
        ...
        //     .attr("cx",blank_size + (width-2*blank_size)*(currentYear - min_year)/(max_year - min_year))
        drag_delta = 0;
        refresh_chart(Math.floor(current_index))
        }
    }
...
function refresh_chart(current_index){
    var bubble = data => d3.pack()
    .size([width, height - offset])
    .padding(2)(d3.hierarchy({ children: selected_data }).sum(function(d){
        if (!d.children){
            // console.log(d.value[current_index][1])
