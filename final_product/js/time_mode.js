            ...
            d3.select(".modal-body").html(function(){
                return (
                    ...
                    <div class='infowindow'>
                      ...
                      <tr>
                          <th>資本金</th>
                          <td>${(d.capital != "None") ? d.capital/100 : "-"}億円</td>
                      </tr>
                      <tr>
                          <th>設立年</th>
                          <td>${(d.found_year != "None") ? d.found_year : "-"}年</td>
                      </tr>
                      <tr>
                          <th>上場年</th>
                          <td>${(d.listed_year != "None") ? d.listed_year : "-"}年</td>
                      </tr>
                      <tr>
                          <th>従業員数</th>
                          <td>${(d.num_of_employees != "None") ? d.num_of_employees : "-"}人</td>
                      </tr>
                      <tr>
                          <th>平均残業時間</th>
                          <td>${(d.overtime != "None") ? d.overtime : "-"}h</td>
                      </tr>
                      <tr>
                          <th>有休消化率</th>
                          <td>${(d.paid_holiday != "None") ? d.paid_holiday : "-"}%</td>
                      </tr>
                      <tr>
                          <th>平均年収</th>
                          <td>${(d.income != "None") ? d.income/10 : "-"}万円</td>
                      </tr>
                      <tr>
                          <th>平均年齢</th>
                          <td>${(d.age != "None") ? d.age : "-"}歳</td>
                      </tr>
...
(async () => {
    ...
    categorySumCap = caluculateSumCap(data);
    categoryMinMax = calculateMinMax(data);
    console.log(categorySumCap);
    console.log(categoryMinMax);
    console.log(selectedMinMax(categoryMinMax, selected_categories));
    generateChart(data);
})();

function caluculateSumCap(data){
    let categorySumCap = [];

    main_categories.forEach(category => {
        let hash = {};
        hash.category = category;
        let tmp = new Array(data[0].value.length) 
        for(let i = 0; i < data[0].value.length; ++i){
            tmp[i] = 0;
        }
        hash.value = tmp
        categorySumCap.push(hash);
    })

    data.forEach(company => {
        for(let i = 0; i < categorySumCap.length; i++){
            if(categorySumCap[i].category == company.category_new){
                for(let j = 0; j < data[0].value.length; ++j ){
                    categorySumCap[i].value[j] += parseFloat(company.value[j][1]);
                }
            }
        }
    })
    return categorySumCap;
}
...
function calculateMinMax(data){
    ...
    main_categories.forEach(category => {
        let tmp = [999999, 0];
        categoryMinMax[category] = tmp;
    })

    data.forEach(company => {
        company.value.forEach(value => {
            if(value[1] != "0")
            {
                if(parseFloat(value[1]) < categoryMinMax[company.category_new][0]){
                    categoryMinMax[company.category_new][0] = parseFloat(value[1]);
                }
                if(parseFloat(value[1]) > categoryMinMax[company.category_new][1]){
                    categoryMinMax[company.category_new][1] = parseFloat(value[1]);
                }
            }
        })
    })
...
function selectedMinMax(categoryMinMax, selected_categories){
    var MinMax = [99999,0];
    selected_categories.forEach(category => {
        if(categoryMinMax[category][0] < MinMax[0]){
            MinMax[0] = categoryMinMax[category][0]
        }
        if(categoryMinMax[category][1] > MinMax[1]){
            MinMax[1] = categoryMinMax[category][1]
        }
    })
    return MinMax;
}
...
////////////////////////////////////////////
function generateChartCategory(data){
    console.log("hoge");
    var svg = d3.select('#time-mode')
        .style('width', width)
        .style('height', height);
    ...
    var selectedSumCap = [];
    categorySumCap.forEach(data => {
        if(selected_categories.includes(data.category)){
            selectedSumCap.push(data);
        }
    })
    ...
    node
        ...
        .attr("r", function(d){
            var linear_scale = d3.scaleLinear().domain([0.05, 253]).range([radius_min, radius_max]);
            return linear_scale(d.value[Math.floor(current_index)]);
        })
            ...
            d3.forceCollide()
                .radius(
                    function (d) {
                        console.log(d);
                        var linear_scale = d3.scaleLinear().domain([0.05, 253]).range([radius_min, radius_max]);
                        return linear_scale(d.value[Math.floor(current_index)]) + padding;
                    }
                )
                .strength(0.7)
        )
        .force('x', d3.forceX().strength(function(d) {return force_scale(d.value[current_index])}).x(width / 2))
        .force('y', d3.forceY().strength(function(d) {return force_scale(d.value[current_index])}).y(height / 2))
        .force("center", d3.forceCenter(width / 2, height / 2));
    
    //console.log(nodesData)
    simulation
        .nodes(selectedSumCap)
        .on("tick", ticked);
    ...
    const rangeOnChange = (e) =>{
        ...
        yearLabel.text(index_to_str(Math.floor(current_index)));
        svg.selectAll(".node_circle")
            .data(selectedSumCap, function (d) { return d.category })
            .attr("r", function(d){
                var linear_scale = d3.scaleLinear().domain([0.05, 253]).range([radius_min, radius_max]);
                return linear_scale(d.value[Math.floor(current_index)])
            })
